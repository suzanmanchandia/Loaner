<fieldset>
    <legend class="clearfix">
        <div class="row">
            <div class ="col-xs-7">
                Reservation Details : {{ $loan->lid }}
            </div>
            <div class="col-xs-5 text-right">
                @include('reservations.actions', array('item' => $loan))
            </div>
        </div>
    </legend>
...
<fieldset>
    ...
    <dl>
        ...
        <dd>
            @foreach ($equipment as $loan)
            <div class="col-md-6">
                <h4> <a href="javascript:;" data-dialog="yes" data-title="{{{ $loan->model }}}" data-href="{{ route( 'equipment.show', array($loan->equipmentid) ) }}">{{ $loan->model }} {{ $loan->equipmentid }}</a></h4>
                <div class="well">
                    <dl>
                        <dt>Equipment ID</dt>
                        <dd>{{ $loan->equipmentid }}</dd>
                        <dt>Model</dt>
                        <dd>{{ $loan->model }}</dd>
                        <dt>Manufacturer</dt>
                        <dd>{{ $loan->manufacturer }}</dd>
                        <dt>Notes</dt>
                        <dd>{{ $loan->notes }}</dd>
                    </dl>
                </div>
